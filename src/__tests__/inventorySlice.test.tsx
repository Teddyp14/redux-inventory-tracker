import inventoryReducer, { addItem, selectItem, updateItem, deleteItem, purchaseItem, updateInventory } from '../redux/inventorySlice'
...
describe("inventorySlice", () => {
    ...
    test("Should decrement the selectedItem quantity by 1.", () => {
        ...
        expect(updatedState).toEqual({ inventoryList: [testingItem], selectedItem: testingItem3 })
    })

    test("Should change a specified item in inventoryList to the same value as the selectedItem", () => {
        const initialState = {
            inventoryList: [testingItem],
            selectedItem: testingItem3
        }
        const updatedState = inventoryReducer(initialState, updateInventory())
        expect(updatedState).toEqual({ inventoryList: [testingItem3], selectedItem: testingItem3 })
    })
})
